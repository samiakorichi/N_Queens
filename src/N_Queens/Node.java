...
public class Node {
    ...
    public ArrayList<Node> generateNeighborhoods(){
        ...
        for(int i = 0; i < n; i++){
        	// If it is possible to put the integer i at the end of the vector without hurting the game properties,
        	// then it is added as neighbor
            if(isValidOperation(i)){
                ArrayList<Integer> newState = new ArrayList<>(state.getValues()); //creating a copy of the current state
    ...
    private boolean isValidOperation(int x){
        ...
        if(lx == n) return false; //board has already been filled by N queens
        for(int i = 0; i < state.getValues().size(); i++){
            int li = i;
            int ci = state.getValues().get(i);
            if(ci == cx) return false;              //teste s'il y a déjà une pièce qui occupe la colonne de la pièce x
            if(ci - li == cx - lx) return false;    //teste s'il y a déjà une pièce sur la diagonale (principale) de la pièce x
            if(ci + li == cx + lx) return false;    //teste s'il y a déjà une pièce sur la diagonale (secondaire) de la pièce x
        }
        return true;
    }
    ...
    public String toString(boolean showBoard) {
    	//if showBoard = true, returns a String that shows an NxN board for a better visualization of the solution
    ...
    public String showPath(boolean showBoard){
    	//returns the toString of the path from the start node (empty) to the current node
        String res = "";
        Node atual = this;
        while(atual != null) {
